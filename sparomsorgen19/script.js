...
// d3.json("SverigesKommuner.geojson").then(function(geodata){
	...
	// d3.json("kommundata.json").then(function(artikeldata){
		...
		var color = d3.scaleOrdinal()
				.domain(["Kommuner som sparar", "Kommuner som inte sparar", "Oklart/ingen uppgift"])
...
function mousemove(d, i) {
	...
	let markup = `
		...
		<p class='no-margin-bottom'>${d.Kommentar}</p>
		${(() => {
			if (d.Skott < 0) {
				return `
				<p><span class='fet'>Underskott 2018:</span> ${d.Skott.toString().replace(/-/,'')} mkr</p>
				`
			}
			else {
				return`
				<p><span class='fet'>Överskott 2018:</span> ${d.Skott} mkr</p>
				`
			}
			})()}
	`
