...
function clicked(d,i) {
  ...
  let markup = `
    ...
    ${(() => {
      if (d.bildurl != "") {
        return `
       <iframe width="100%" height="auto" src="${d.bildurl}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
        `
      }
      else {
        return`
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    var projection = d3.geoConicEqualArea()
        ...
        .translate([w*0.6, h/1.25]) //translate to center the map in view;
    ...
    Promise.all([map]).then(function(values) {
        ...
        var datafile = d3.json("rekryt.json");
